BEAT_NAME=icube-cybervision-plugin
BEAT_PATH=github.com/CiscoDevNet/icube-cybervision-plugin
...
git-init:
	...
	git commit -m "Add icube-cybervision-plugin"
	git add .travis.yml
...
local:
	CGO_ENABLED=0 go build -o release/linux/amd64/icube-cybervision-plugin
docker:
	GOOS=linux   GOARCH=amd64 CGO_ENABLED=0 go build -o release/linux/amd64/icube-cybervision-plugin
	docker build -t cocreate/icube .
